...
server <- function(input, output, session) {
  ...
  observeEvent(input$allocateCategoryBtn, {
    ...
    if (!is.null(input$datatable_rows_selected)) {
      ...
      selected_data <- data()
      modified_data_val <- selected_data[selected_rows, , drop = FALSE]
      modified_data_val$exp_cat <- category
      modified_data(rbind(modified_data(), modified_data_val))
      data(selected_data[-selected_rows, , drop = FALSE])
    } else {
